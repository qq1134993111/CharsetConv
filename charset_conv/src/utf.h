...
///
		...
		struct utf_traits<CharType, 1>
		{
			...
			static code_point decode(Iterator& p, Iterator e)
			{
				...
				// valid range
				if (UNLIKELY(!is_valid_codepoint(c)))
					return illegal;

				// make sure it is the most compact representation
				if (UNLIKELY(width(c) != trail_size + 1))
					return illegal;
			...
			static code_point decode_valid(Iterator& p)
			{
				...
				if (lead < 224)
					trail_size = 1;
				else if (LIKELY(lead < 240)) // non-BMP rare
					trail_size = 2;
				else
					trail_size = 3;
		...
		struct utf_traits<CharType, 2>
		{
			...
			{
				if (UNLIKELY(current == last))
					return incomplete;
				uint16_t w1 = *current++;
				if (LIKELY(w1 < 0xD800 || 0xDFFF < w1))
				{
					return w1;
				}
				if (w1 > 0xDBFF)
